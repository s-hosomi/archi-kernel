...
<p align="center">
  <img src="assets/hero.png?v=5" alt="セットバックした屋上テラス、スチールパーゴラ、丸柱コロネード、窓グリッドのファサードとコア壁を持つ 3 層 RC オフィスビル。カーネル CSG としてモデル化し Three.js ビューアで描画したもの" width="850">
</p>
...
<p align="center">
  <img src="assets/section-sweep.gif?v=5" alt="オフィスモデルを断面平面で上から下へスイープする様子。朱色のキャップと輪郭は毎フレーム、カーネルの閉形式 section() で計算している" width="850">
</p>
...
<p align="center">
  <img src="assets/curved-hero.png?v=1" alt="円筒ヴォールト屋根、球面ガラスドーム、銅色の円錐ルーフランタンを持つ、別画面の曲面パネルホールデモ" width="850">
</p>
...
<p align="center">
  <img src="assets/curved-sweep.gif?v=1" alt="曲面パネルホールを断面平面でスイープする様子。CSG 支持部材にはカーネル計算の断面キャップが付き、解析曲面パネルは表示用にクリップされる" width="850">
</p>

*曲面ホールは既存オフィスを汚さず、汎用の曲面パネル表示レイヤを別デモとして見せます。矩形 UV トップライト付きの厚み付き円筒ヴォールトは `CylinderPanel`、UV オキュラス付きのロタンダドームは `SpherePanel`、スロット付きルーフランタンは `ConePanel` です。いずれも Three.js で手描きしたメッシュではなく、wasm API に渡した JSON から生成しています。*
...
`?demo=curved` では、別画面の曲面パネルホールを読み込みます。オフィスの構成をきれいに保ったまま、矩形 UV トップライトを持つ厚み付き円筒ヴォールト、丸い UV オキュラスを持つ球面ロタンダドーム、スロット付き円錐ルーフランタンを、同じ wasm 境界から解析曲面パネルとして登録します。
...
# http://localhost:8741/?demo=curved で別画面の曲面パネルホールを表示
```

断面スライダーは、現在の分担をそのまま見せるデモです。動かすたびにビューアは `section_all()` を呼び、返ってきた CSG レイヤの閉形式プロファイル(穴も円弧も含む)から朱色のキャップを作り直します。Three.js のクリッピングがやっているのは、平面より上を非表示にすることだけです。曲面パネルホールでは解析曲面パネルも表示用にクリップされますが、まだ B-rep/section evaluator の一部ではありません。
...
- wasm の解析曲面パネル表示レイヤ:矩形 UV 開口つき厚み付き円筒パネル、UV オキュラスつき球面パネル、表示用円錐台パネル

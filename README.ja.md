...
<p align="center">
  <img src="assets/hero.png?v=6" alt="セットバックした屋上テラス、スチールパーゴラ、丸柱コロネード、窓グリッドのファサードとコア壁を持つ 3 層 RC オフィスビル。カーネル CSG としてモデル化し Three.js ビューアで描画したもの" width="850">
</p>
...
<p align="center">
  <img src="assets/section-sweep.gif?v=6" alt="オフィスモデルを断面平面で上から下へスイープする様子。朱色のキャップと輪郭は毎フレーム、カーネルの閉形式 section() で計算している" width="850">
</p>

*どちらも、デフォルトの CSG オフィスデモをビューアで描いたものです。CSG ソリッドは watertight にテッセレーションされ、朱色の断面キャップと輪郭は、断面平面を動かすたびにカーネルの閉形式 `section()` から作り直しています。*

<p align="center">
  <img src="assets/curved-hero.png?v=2" alt="アヤソフィア風の曲面パネル建築。中央ドーム、東西の半ドーム、4 本のミナレット、重いバットレスを持ち、解析曲面上の UV trim で開口を抜いている" width="850">
</p>

<p align="center">
  <b><a href="https://s-hosomi.github.io/archi-kernel/?demo=curved">▶ 曲面パネルデモ</a></b> — 同じ wasm ビューアで、別の曲面建築モデルを読み込みます
</p>

<p align="center">
  <img src="assets/curved-orbit.gif?v=2" alt="アヤソフィア風の曲面パネル建築を 360 度から見渡す GIF。ドーム、半ドーム、ミナレット、曲面 UV trim 開口を全方向から確認できる" width="850">
</p>

*曲面デモは既存オフィスを汚さず、汎用の曲面パネル表示レイヤを別デモとして見せます。厚み付き `SpherePanel` のドーム/半ドーム、厚み付き `CylinderPanel` のヴォールト、`ConePanel` のミナレット屋根を wasm JSON で登録しています。ドームの採光スロット、上部の小開口、半ドームの開口列、ヴォールトのトップライトは、Three.js で手描きしたメッシュではなく解析曲面上の UV trim です。*
...
`viewer/` はビルド不要の Web アプリです。カーネルを WebAssembly にコンパイルし(`wasm/` は薄い wasm-bindgen アダプタで、ジオメトリはフラットな型付き配列、CSG はカーネル自身の serde JSON、解析曲面パネルは wasm 境界の JSON で受け渡します)、ES モジュールの Three.js シーンと組み合わせています。デフォルトデモでは **3 層の RC オフィスビルを JavaScript 側で CSG モデルとして組み立てます**(約 160 部材)。最上階をセットバックさせた屋上テラスとスチール H 形鋼のパーゴラ、正面の丸柱コロネード、エントランスとキャノピーを持つ窓グリッドのファサード、全フロアを貫く階段・エレベーターコア、スリーブを通した大梁 — そして数量拾いの控除チェーン(柱 ▷ 大梁 ▷ 小梁 ▷ スラブ)まで、実務の構成をそのまま再現しています。

`?demo=curved` では、アヤソフィア風の曲面パネルスタディを読み込みます。オフィスの構成をきれいに保ったまま、UV trim 開口を持つ厚み付き球面中央ドーム、東西半ドームの開口列、トップライト付きの側廊ヴォールト、円錐ミナレット屋根を、同じ wasm 境界から解析曲面パネルとして登録します。
...
# http://localhost:8741/?demo=curved で別画面の曲面パネルスタディを表示
```

断面スライダーは、現在の分担をそのまま見せるデモです。動かすたびにビューアは `section_all()` を呼び、返ってきた CSG レイヤの閉形式プロファイル(穴も円弧も含む)から朱色のキャップを作り直します。Three.js のクリッピングがやっているのは、平面より上を非表示にすることだけです。曲面パネルスタディでは解析曲面パネルも表示用にクリップされますが、まだ B-rep/section evaluator の一部ではありません。
...
- wasm の解析曲面パネル表示レイヤ:多角形/矩形 UV 開口つき厚み付き円筒・球面パネル、表示用円錐台パネル

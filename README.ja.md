...
<p align="center">
  <img src="assets/hero.png?v=4" alt="厚み付き円筒ヴォールト屋根、球面ドーム、円錐台キャノピーを持つ 3 層 RC オフィスビル。CSG ソリッドと解析曲面パネルを Three.js ビューアで描画したもの" width="850">
</p>
...
<p align="center">
  <img src="assets/section-sweep.gif?v=4" alt="断面平面を上から下へスイープする様子。朱色のキャップと輪郭は毎フレーム、カーネルの閉形式 section() で計算し、解析曲面パネルのレイヤは表示用にクリップしている" width="850">
</p>

*どちらも、カーネル自身が計算した結果をビューアで描いています。CSG ソリッドは watertight なテッセレーション、朱色の断面は閉形式 `section()` の出力です。さらに wasm JSON 経由の汎用曲面パネルレイヤとして、`CylinderPanel` / `SpherePanel` / `ConePanel` の表示メッシュも重ねています。ヴォールト屋根のトップライト、ドームの丸いオキュラス、円錐台キャノピーのスロットは、Three.js で手描きした形ではなく解析曲面上の UV trim です。*
...
`viewer/` はビルド不要の Web アプリです。カーネルを WebAssembly にコンパイルし(`wasm/` は薄い wasm-bindgen アダプタで、ジオメトリはフラットな型付き配列、CSG はカーネル自身の serde JSON、解析曲面パネルは wasm 境界の JSON で受け渡します)、ES モジュールの Three.js シーンと組み合わせています。デモでは **3 層の RC オフィスビルを JavaScript 側で CSG モデルとして組み立てます**(約 160 部材)。最上階をセットバックさせた屋上テラスとスチール H 形鋼のパーゴラ、正面の丸柱コロネード、エントランスとキャノピーを持つ窓グリッドのファサード、全フロアを貫く階段・エレベーターコア、スリーブを通した大梁 — そして数量拾いの控除チェーン(柱 ▷ 大梁 ▷ 小梁 ▷ スラブ)まで、実務の構成をそのまま再現しています。さらに同じ wasm 境界から、矩形トップライトを持つ厚み付き円筒ヴォールト、丸い UV オキュラスを持つ球面テラスドーム、スロット付き円錐台キャノピーを解析曲面パネルとして登録します。あとはカーネルの仕事です:評価、CSG ソリッドの watertight メッシュ化、曲面パネルのテッセレーション、CSG レイヤのライブ断面(キャップはカーネルが計算)、HUD のコンクリート量集計。
...
断面スライダーは、現在の分担をそのまま見せるデモです。動かすたびにビューアは `section_all()` を呼び、返ってきた CSG レイヤの閉形式プロファイル(穴も円弧も含む)から朱色のキャップを作り直します。Three.js のクリッピングがやっているのは、平面より上を非表示にすることだけです。曲面パネルはこのフェーズでは表示用にクリップされますが、まだ B-rep/section evaluator の一部ではありません。
